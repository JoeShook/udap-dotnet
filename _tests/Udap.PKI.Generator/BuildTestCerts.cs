...
public class MakeCa
{
    ...
    private static string SureFhirLabsCaPublicCertHosted { get; } =
        $"http://crl.fhircerts.net/certs/SureFhirLabs_CA.cer";

    private static string SureFhirLabsAnchorPublicCertHosted { get; } =
        "http://crl.fhircerts.net/anchors/SureFhirLabs_Anchor.cer";
    ...
    private static string LocalhostDevCertPath { get; } = $"{BaseDir}/certstores/Kestrel/Localhost";
    ...
    public void MakeCaWithAnchorUdapAndSSLForDefaultCommunity()
    {
        ...
        using (RSA anchor = RSA.Create(4096))
        {
                       ...
                       DateTimeOffset.UtcNow.AddYears(10)))
            {
                ...
                File.WriteAllBytes($"{SureFhirLabsCertStore}/SureFhirLabs_CA.cer",
                ...
                subAltNameBuilder.AddUri(
                    new Uri("udap://surefhir.labs")); // My way of embedding a community uri in anchor cert
                           ...
                           new ReadOnlySpan<byte>(new byte[] { 1, 2, 3, 4 })))
                {
                    ...
                    File.WriteAllBytes($"{SurefhirlabsUdapAnchors}/SureFhirLabs_Anchor.cer",
                        anchorPem.Select(c => (byte)c).ToArray());
                    ...
                    authorityInfoAccessBuilder.AdCertificateAuthorityIssuerUri(
                        new Uri(SureFhirLabsAnchorPublicCertHosted));
                               ...
                               new byte[] { 1, 2, 3, 4 }))
                    {
                        ...
                        File.WriteAllBytes($"{SurefhirlabsUdapIssued}/WeatherApiClient.cer",
                               ...
                               new byte[] { 1, 2, 3, 4 }))
                    {
                        ...
                        char[] clientPem = PemEncoding.Write("CERTIFICATE", clientCert.RawData);
                        File.WriteAllBytes($"{SurefhirlabsUdapIssued}/fhirlabs.net.client.cer",
                            clientPem.Select(c => (byte)c).ToArray());
                    }
                               ...
                               new byte[] { 1, 2, 3, 4 }))
                    {
                        ...
                        File.WriteAllBytes($"{SureFhirLabsSslWeatherApi}/weatherapi.lab.cer",
                            certificatePem.Select(c => (byte)c).ToArray());
                    }
                               ...
                               new byte[] { 1, 2, 3, 4 }))
                    {
                        ...
                        char[] certificatePem = PemEncoding.Write("CERTIFICATE", clientCert.RawData);
                        File.WriteAllBytes($"{SureFhirLabsSslFhirLabs}/fhirlabs.net.cer",
                            certificatePem.Select(c => (byte)c).ToArray());
                    }
                               ...
                               new byte[] { 1, 2, 3, 4 }))
                    {
                        ...
                        char[] certificatePem = PemEncoding.Write("CERTIFICATE", clientCert.RawData);
                        File.WriteAllBytes($"{SureFhirLabsSslIdentityServer}/securedcontrols.net.cer",
                            certificatePem.Select(c => (byte)c).ToArray());
                    }
                    ...
                    var anchorCrl =
                        crlAnchorGen.Generate(new Asn1SignatureFactory("SHA256WithRSAEncryption", anchorAkp,
                            anchorRandom));

                    SurefhirlabsCrl.EnsureDirectoryExists();
                    File.WriteAllBytes($"{SurefhirlabsCrl}/{SureFhirLabsAnchorPkcsFileCrl}",
                        anchorCrl.GetEncoded());
                       ...
                       DateTimeOffset.UtcNow.AddYears(10)))
            {
                           ...
                           new ReadOnlySpan<byte>(new byte[] { 1, 2, 3, 4 })))
                {
                               ...
                               new byte[] { 1, 2, 3, 4 }))
                    {
                        ...
                        File.WriteAllBytes($"{LocalhostCertStore}/anchorLocalhostCert.cer",
                            anchorPem.Select(c => (byte)c).ToArray());
                        ...
                        char[] clientPem = PemEncoding.Write("CERTIFICATE", clientCert.RawData);
                        File.WriteAllBytes($"{LocalhostCertStore}/fhirLabsApiClientLocalhostCert.cer",
                            clientPem.Select(c => (byte)c).ToArray());
    ...
    /// <summary>
    /// When working with Windows and Linux through your WSL instance the typical Dev cert for localhost will not work on
    /// Linux.  Linux want's the certificate Basic Constraints, Subject Type set to CA.
    ///
    /// Run this script and then the following commands:
    ///
    /// 
    /// </summary>
    [Fact]
    public void BuildSharedLocalhostCertforWSLLinuxandWindows()
    {
        using (RSA parent = RSA.Create(4096))
        {
            var parentReq = new CertificateRequest(
            ...
            parentReq.CertificateExtensions.Add(new X509Extension("1.3.6.1.4.1.311.84.1.1", new byte[] {0x0002}, false));
            ...
            {
                var parentBytes = caCert.Export(X509ContentType.Pkcs12, "udap-test");
                LocalhostDevCertPath.EnsureDirectoryExists();
                File.WriteAllBytes($"{LocalhostDevCertPath}/localhostDevCert.pfx", parentBytes);
                char[] caPem = PemEncoding.Write("CERTIFICATE", caCert.RawData);
                File.WriteAllBytes($"{LocalhostDevCertPath}/localhostDevCert.cer",
                    caPem.Select(c => (byte)c).ToArray());
