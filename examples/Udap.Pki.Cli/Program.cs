...
public class PublicCertStoreConfig
{
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        bool isProduction = args.Any(a => a.Equals("--prod", StringComparison.OrdinalIgnoreCase) || a.Equals("--production", StringComparison.OrdinalIgnoreCase));

        if (isProduction)
        {
            AnsiConsole.MarkupLine("[bold green]Running in PRODUCTION mode[/]");
            // Place any production-specific logic or configuration here
        }
        else
        {
            AnsiConsole.MarkupLine("[bold yellow]Running in TEST mode (default)[/]");
            // Place any test-specific logic or configuration here
        }
        ...
        await storage.DownloadObjectAsync(gcpPkiConfig.CaBucket, gcpPkiConfig.CaP12Path, caP12Stream);
        ...
        await storage.DownloadObjectAsync(gcpPkiConfig.CaBucket, gcpPkiConfig.CaCrlPath, crlStream);
        ...
        if (isProduction)
        {
            ...
            await storage.UploadObjectAsync(gcpPkiConfig.CaBucket, gcpPkiConfig.CaCrlPath, "application/pkix-crl",
                uploadStream);

            AnsiConsole.MarkupLine("[green]New CRL uploaded successfully![/]");
        
            var publicCrlObjectName = $"{publicCertStoreConfig.CrlPath}/{Path.GetFileName(gcpPkiConfig.CaCrlPath)}";
    ...
    public static byte[] GenerateNewCrl(X509Certificate2 caCert, string caPassword, byte[] previousCrlBytes)
    {
        ...
        {
            // Try to get the CrlReason extension from the entry
            var reasonExt = entry.GetExtensionValue(X509Extensions.ReasonCode);

            if (reasonExt != null)
            {
                // Add the entry with the reason code extension
                crlGen.AddCrlEntry(
                    entry.SerialNumber,
            ...
            {
                // Add the entry without extensions
                crlGen.AddCrlEntry(
                    entry.SerialNumber,
                    entry.RevocationDate,
                    null
